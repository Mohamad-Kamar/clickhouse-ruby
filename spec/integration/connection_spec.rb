...
RSpec.describe "Connection Integration", :integration do
  ...
  describe "authentication", if: ENV.fetch("CLICKHOUSE_USER", nil) do
    it "authenticates with provided credentials" do
      result = client.execute("SELECT currentUser() as user")
      expect(result.first["user"]).to eq(ENV.fetch("CLICKHOUSE_USER", nil))
    end
  end
  ...
  describe "large result sets" do
    ...
    it "streams results efficiently" do
      ...
      count = 0
      client.execute("SELECT number FROM system.numbers LIMIT 10000").each do |_row|
        count += 1
      end
